...
export default function Register() {
    ...
    const create = async (data) => {
        ...
        try {
            ...
            // const response = await fetch('https://cautious-sniffle.netlify.app/.netlify/functions/api/createform', {
            // method: 'POST',
            //     headers: {
            ...
            // }));
            document.getElementById('creteform').reset();
            setsendDate(false)
        } catch (error) {
            ...
            //     setError('email must be uniques');
            // } else {
    ...
    return (
        <div className="w-full max-w-sm my-10 p-6 m-auto mx-auto bg-white rounded-lg shadow-md dark:bg-gray-800">
            ...
            >
                ...
                {Error && <p>{Error}</p>}
                <div className="mt-6">
                    <button className="w-full px-6 border py-2.5 text-sm font-medium tracking-wide text-white capitalize transition-colors duration-300 transform bg-gray-800 rounded-lg hover:bg-gray-700 focus:outline-none focus:ring focus:ring-gray-300 focus:ring-opacity-50">
                        {SendDate ? "Sending Data" : "Create One"}
            ...
            <p className="mt-8 text-xs font-light text-center text-gray-400"> You have an account? <Link to="/login" className="font-medium text-gray-700 dark:text-gray-200 hover:underline">Login</Link></p>
        </div>
    )
}
